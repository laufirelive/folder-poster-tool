...
为确保门禁接入后实际抠像链路可用，增加固定验收样本：

1. 输入图片：`/Users/liujiahao/Documents/testimge/OIP-2190781769.jpg`
2. 前置条件：模型已在本地目录完成安装（若未安装，必须先通过下载页完成下载）
3. 操作路径：启动应用 -> 通过门禁 -> 进入素材页并选择该图片 -> 进入抠像页执行
4. 期望结果：
   - 抠像流程正常完成，不出现“模型缺失”类错误分支
   - 生成可预览的 RGBA 抠像结果图
   - 可在抠像页右侧结果卡片中正常预览
...
6. 指定样本 `/Users/liujiahao/Documents/testimge/OIP-2190781769.jpg` 能成功产出抠像结果并可预览。
